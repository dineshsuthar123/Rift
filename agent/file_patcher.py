...
def apply_fix_to_file(repo_path: str, fix: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        print(f"[PATCHER] Error reading {file_path}: {e}")
        return False
    ...
    if line_number < 1 or line_number > len(lines):
        print(f"[PATCHER] Line {line_number} out of range for {fix['file_path']} (has {len(lines)} lines)")
        return False
    ...
    try:
        ...
        print(f"[PATCHER] Applied fix to {fix['file_path']} line {line_number}")
        return True

    except Exception as e:
        print(f"[PATCHER] Error applying fix to {file_path}: {e}")
        return False
